...
import { accountManagers } from '../../data/mockData';
    ...
    <div className="fade-in">
      ...
      <div className="card" style={{ marginTop: 'var(--space-6)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 'var(--space-4)' }}>
          {[
            { label: 'Total Managers', value: accountManagers.length },
            { label: 'Active', value: accountManagers.filter((m) => m.status === 'active').length },
            { label: 'At Capacity', value: accountManagers.filter((m) => m.status === 'at capacity').length },
            { label: 'Inactive', value: accountManagers.filter((m) => m.status === 'inactive').length },
            { label: 'Total Clients', value: accountManagers.reduce((s, m) => s + m.activeClients, 0) },
            { label: 'Total Capacity', value: accountManagers.reduce((s, m) => s + m.capacity, 0) },
          ].map((s) => (
            <div key={s.label}>

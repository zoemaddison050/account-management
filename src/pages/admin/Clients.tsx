...
import { allClients, accountManagers } from '../../data/mockData';
    ...
    <div className="fade-in">
      ...
      <div style={{ display: 'flex', gap: 'var(--space-3)', marginBottom: 'var(--space-5)', flexWrap: 'wrap' }}>
        ...
        <select className="form-control" style={{ maxWidth: '220px' }} value={managerFilter} onChange={(e) => setManagerFilter(e.target.value)}>
          ...
          {accountManagers.filter((m) => m.status !== 'inactive').map((m) => (
            ...
            <tbody>
              {filtered.map((c) => (
                <tr key={c.id}>
                  ...
                  <td style={{ fontSize: '0.85rem' }}>{c.portfolios.length > 0 ? `${c.portfolios.length} portfolio${c.portfolios.length > 1 ? 's' : ''}` : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>

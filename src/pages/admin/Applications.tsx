...
import { applications, statusColors, formatDateTime, relativeTime } from '../../data/mockData';
...
export default function Applications() {
  ...
  const filtered = applications.filter((a) => {
    ...
    const matchesSearch = !search || a.applicantName.toLowerCase().includes(search.toLowerCase()) || a.reference.toLowerCase().includes(search.toLowerCase()) || a.email.toLowerCase().includes(search.toLowerCase());
  ...
  const stats = {
    ...
    pending: applications.filter((a) => ['Inquiry submitted', 'Application received', 'Under review', 'Information requested', 'Approval pending'].includes(a.status)).length,
    approved: applications.filter((a) => ['Approved — activation pending', 'Active client'].includes(a.status)).length,
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--space-5)' }}>
              ...
              <button onClick={() => setSelected(null)} className="btn btn-ghost" style={{ padding: '0.3rem' }} aria-label="Close detail panel">
            ...
            <div style={{ marginTop: 'var(--space-4)' }}>
              <textarea className="form-control" placeholder="Add an internal note..." rows={3} style={{ marginBottom: 'var(--space-3)' }} />
              <button className="btn btn-secondary btn-block" style={{ fontSize: '0.85rem' }}>Add Note (logged to audit trail)</button>
            </div>

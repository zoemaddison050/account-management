...
import { demoClient, formatCurrency, formatDate, portfolioHistory } from '../../data/mockData';
...
export default function Portfolio() {
  ...
  const portfolio = demoClient.portfolios[selectedIdx];
  ...
  // Allocation by type
  const typeAllocation = portfolio.holdings.reduce<Record<string, number>>((acc, h) => {
  ...
  const portfolioHistoryFiltered = portfolioHistory.filter((s) => s.portfolioId === portfolio.id);
  const hasHistory = portfolioHistoryFiltered.length > 0;
  const historyPoints = portfolioHistoryFiltered.map((s, i) => ({
    x: (i / (portfolioHistory.length - 1)) * 100,
    y: s.totalValue,
    date: s.date,
  }));
  ...
  return (
    <div className="fade-in">
      ...
      {/* Portfolio selector */}
      {demoClient.portfolios.length > 1 && (
        <div style={{ display: 'flex', gap: 'var(--space-3)', marginBottom: 'var(--space-5)' }}>
          {demoClient.portfolios.map((p, i) => (
            <button
              key={p.id}
      ...
      <div className="card" style={{ marginBottom: 'var(--space-5)' }}>
        <h3 style={{ fontSize: '1.05rem', marginBottom: 'var(--space-4)' }}>Value History (6 months)</h3>
        {hasHistory ? (
          <>
